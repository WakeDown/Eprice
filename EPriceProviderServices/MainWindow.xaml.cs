...
namespace EPriceProviderServices
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private async Task GetOldiData()
        {
            ...
            {
                Interlocked.Decrement(ref _catalogLoaderCounter);
                CatalogsLoadComplete();
            }
        }
        ...
        private async Task GetUn1tData()
        {
        ...
        private async void UpdateUn1tCategoryView(int id)
        {
            ...
            Dispatcher.Invoke(() =>
            {
                ...
                else
                {
                    ...
                    if (viewModel != null)
                    {
                        viewModel.Item.SetHighlight(true, true);
                    }
                }
        ...
        private async void RefreshServiceVariable(IDataService service)
        {
            ...
            {
                Log.WriteToLog(LogSender + " RefreshServiceVariable(" + service.Provider + ") ", "Ошибка: " + ex.Message);
                WriteToLog("Ошибка при обновлении справочников " + service.Provider);
            }
            finally
            {
                Interlocked.Decrement(ref _directLoaderCounter);
                ServiceDirectRefreshed();
            }
        }

        private void ServiceDirectRefreshed()
        {
            if (_directLoaderCounter == 0)
            {
                WriteToLog("Все справочники обновлены");
                if (!GetProductsMultiThread(_merlionData, _merlionService, ProviderType.Merlion))
                    GetMerlionProducts();
                if (!GetProductsMultiThread(_treolanData, _treolanService, ProviderType.Treolan))
                    GetTreolanProducts();
                if (!GetProductsMultiThread(_oldiData, _oldiService, ProviderType.OLDI))
                    GetOldiProducts();
                GetOcsProducts();
            }
        }

        private bool GetProductsMultiThread(DataLevelBase dataLevel, IDataService service, ProviderType provider)
        {
            var isStarted = false;
            try
            {
            ...
            {
                Log.WriteToLog(LogSender, " GetProductsMultiThread(" + provider + ") Ошибка: " + ex.Message);
                WriteToLog("Ошибка при загрузке товаров (многопоточно) " + provider);
            }
            return isStarted;
        }

        private async Task GetMerlionProducts()
        {
            try
            {
            ...
            {
                Log.WriteToLog(LogSender + " GetMerlionProducts() ", "Ошибка: " + ex.Message);
                WriteToLog("Ошибка при загрузке товаров Merlion");
            }
            finally
            {
                Interlocked.Decrement(ref _productLoaderCounter);
                LoadProductComplete();
            }
        }

        private async Task GetTreolanProducts()
        {
            try
            {
            ...
            {
                Log.WriteToLog(LogSender + " GetTreolanProducts() ", "Ошибка: " + ex.Message);
        ...
        private async Task GetOcsProducts()
        {
            try
            {
            ...
            {
                Log.WriteToLog(LogSender + " GetOcsProducts() ", "Ошибка: " + ex.Message);
        ...
        private async Task GetOldiProducts()
        {
            try
            {
            ...
            {
                Log.WriteToLog(LogSender + " GetOldiProducts() ", "Ошибка: " + ex.Message);
        ...
        {
            var isComplete = _productLoaderCounter == 0;
            if (isComplete)
            {
        ...
        private async void GetAllProviderStocks()
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    GetMerlionStocks();
                    if (!GetStocksMultiThread(_oldiData, _oldiService, ProviderType.OLDI))
                    GetOldiStocks();
                }
                catch (Exception ex)
                {
        ...
        private void StocksLoadMultiThreadComplete(ProviderType provider, List<StockServiceBase> stocks)
        {
            ...
            {
                Interlocked.Decrement(ref _priceLoaderCounter);
                LoadStocksComplete();
            }
        }

        private async Task GetMerlionStocks()
        {
            try
            {
                WriteToLog("Стартовала загрузка цен Merlion");
                var list = await GetStocks(_merlionData, _merlionService, ProviderType.Merlion);
                if (list.Any())
                {
            ...
            {
                Log.WriteToLog(LogSender + " GetMerlionStocks() ", "Ошибка: " + ex.Message);
        ...
        private async Task GetOldiStocks()
        {
            try
            {
                WriteToLog("Стартовала загрузка цен OLDI");
                var list = await GetStocks(_oldiData, _oldiService, ProviderType.OLDI);
                if (list.Any())
                {
            ...
            {
                Log.WriteToLog(LogSender + " GetOldiStocks() ", "Ошибка: " + ex.Message);
        ...
        {
            var isComplete = _priceLoaderCounter == 0;
            if (isComplete)
            {
                ...
                try
                {
                    ...
                    foreach (var provider in providers)
                    {
                        if (_providerProducts.ContainsKey(provider))
                        {
                            ...
                            if (products.Any())
                            {
                                var newProducts = db.SaveProducts(products, provider);
        ...
        private async Task GetNewProductsProperties()
        {
            ...
            if (isPropertiesLoad)
            {
                Dispatcher.Invoke(() =>
                {
                    ...
                    PropertiesLoadCount.Content = "Осталось: " + maximum;
                });
        ...
        private void UpdatePropertyProgress()
        {
            Dispatcher.Invoke(() =>
            {
                ...
                if (tag != null)
                {
                    ...
                    PropertiesLoadCount.Content = "Осталось: " + count;
                }
            });
        }
        ...
        private void PropertiesLoadMultiThreadComplete(ProviderType provider, List<PropertyProductBase> properties)
        {
            try
            {
                ...
                WriteToLog("Характеристики " + provider + " загружены");
            }
            finally
            {
                Interlocked.Decrement(ref _propertyLoaderCounter);
                LoadPropertiesComplete();
            }
        }
        ...
        {
            var isComplete = _propertyLoaderCounter == 0;
            if (isComplete)
            {
                try
                {
                    ...
                    await SaveProperiesProductsInStorage();
                }
                finally
                {
                    _loadingData = false;
        ...
        private async Task SaveProperiesProductsInStorage()
        {
            WriteToLog("Сохранение полученных характеристик в хранилище...");
            await Task.Run(() =>
            {
                var provider = ProviderType.Merlion;
                try
                {
                    var db = new DbEngine();
                    foreach (var providerDict in _providerNewProducts.Keys)
                    {
                        ...
                        if (products.Any())
                        {
                            ...
                            {
                                db.SaveProductsProperties(productWithProperties, provider);
                                WriteToLog("Сохранено " + productWithProperties.SelectMany(x => x.Properties).Count() +
                                           " характеристик " + provider);
                            }
                        }
                    }
                }
                ...
                {
                    Log.WriteToLog(LogSender + " SaveProperiesProductsInStorage(" + provider + ") ",
                        "Ошибка: " + ex.Message);
                    WriteToLog("Ошибка при сохранении характеристик " + provider);
                }
            });
            _providerProducts.Clear();
            _providerNewProducts.Clear();
