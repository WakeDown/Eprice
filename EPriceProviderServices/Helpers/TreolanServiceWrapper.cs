        ...
        public async Task<List<PropertyProductBase>> GetPropertiesForProductAsync(string articul)
        {
            ...
            try
            {
                ...
                var node = await Task.Run(() =>
                {
                    ...
                    {
                        _productProxy = new B2BWebService();
                        result = _productProxy.ProductInfoV2(_login, _password, articul);
                    }
                    catch (Exception ex)
                    {
                        _productProxy = new B2BWebService();
                        Log.WriteToLog(_logSender + " GetProductsPropertiesAsync(" + articul + ") ", "Ошибка: " + ex.Message);
                        throw;
                    }
                    return result;
                });
                ...
                foreach (var productId in productsId)
                {
                    var node = await Task.Run(() =>
                    {
                        ...
                        try
                        {
                            ...
                            result = _productProxy.ProductInfoV2(_login, _password, productId);
                        }
                        catch (Exception ex)
                        {
                            proxy = new B2BWebService();
